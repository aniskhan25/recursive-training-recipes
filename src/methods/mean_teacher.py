    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for images, _ in unlabeled_loader:
                ...
                t_probs = torch.softmax(teacher(images), dim=1)
                s_probs = torch.softmax(student(images), dim=1)
                disagreement_total += torch.mean(torch.abs(t_probs - s_probs)).item() * images.size(0)
                disagreement_count += images.size(0)
